...
def _monitor_lock(lock_path: Path):
    ...
    try:
        ...
        raise RuntimeError(f"trade monitor is already running or lock is held: {lock_path}") from exc
    finally:
        try:
...
app = typer.Typer(
    name="TradingAgentsLegacy",
    help="Compatibility command registry; use cli.main:app as the public CLI.",
    add_completion=True,
)
...
class _TradingAgentsGraphRunner:
    ...
    def run(
        ...
        if isinstance(final_state, dict):
            plan = final_state.get("conditional_trade_plan") or {}
            if isinstance(plan, dict):
                plan_id = plan.get("plan_id")
        return run_id, final_signal, confidence, plan_id
...
def trade_monitor(
    ...
    regular_hours_only: bool = typer.Option(False, help="Skip trigger checks outside 09:30-16:00 ET weekdays."),
    no_heartbeat: bool = typer.Option(False, help="Do not write monitor heartbeat events."),
    max_iterations: Optional[int] = typer.Option(None, help="Optional loop count limit for supervised smoke runs."),
    lock_path: Optional[str] = typer.Option(None, help="Optional lock file path for long-running monitor."),
    no_lock: bool = typer.Option(False, help="Disable long-running monitor single-instance lock."),
) -> None:
    ...
    if once or interval_seconds <= 0:
        _ad_print(
            ...
            service.run_once(
                symbols=symbols,
                respect_market_hours=regular_hours_only,
                heartbeat=not no_heartbeat,
            ),
        )
        return
    service.config["trade_monitor_respect_market_hours"] = regular_hours_only
    resolved_lock_path = (
    ...
    try:
        if no_lock:
            ...
            with _monitor_lock(resolved_lock_path):
                service.run_forever(
                    interval_seconds=interval_seconds,
                    symbols=symbols,
                    respect_market_hours=regular_hours_only,
        ...
        _ad_print("trade_monitor", {"error": str(exc), "lock_path": str(resolved_lock_path)})
        raise typer.Exit(2) from exc


@app.command("trade-plan-list")
def trade_plan_list(
    status: Optional[str] = typer.Option(None, help="Comma-separated plan statuses, e.g. active,rejected,executed."),
    symbol: Optional[str] = typer.Option(None, help="Optional single symbol filter."),
    limit: int = typer.Option(50, help="Maximum plans to print."),
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
) -> None:
    ...
    _ad_print(
        "trade_plan_list",
        [summarize_plan(plan, repository) for plan in repository.list_plans(statuses=statuses, symbols=symbols, limit=limit)],
    )


@app.command("trade-plan-show")
def trade_plan_show(
    ...
    if not plan:
        raise typer.BadParameter(f"unknown plan_id: {plan_id}")
    _ad_print("trade_plan_show", summarize_plan(plan, repository, include_events=True, include_validations=True))


@app.command("trade-plan-events")
def trade_plan_events(
    plan_id: str = typer.Option(..., help="Trade plan id."),
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
) -> None:
    ...
    if db_path:
        config["trade_lifecycle_db_path"] = db_path
    repository = TradePlanRepository(config.get("trade_lifecycle_db_path"))
    _ad_print("trade_plan_events", repository.list_events(plan_id))


@app.command("trade-plan-health")
def trade_plan_health(
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
) -> None:
    from tradingagents.trade_lifecycle import TradePlanRepository, plan_health

    config = DEFAULT_CONFIG.copy()
    if db_path:
        config["trade_lifecycle_db_path"] = db_path
    repository = TradePlanRepository(config.get("trade_lifecycle_db_path"))
    _ad_print("trade_plan_health", plan_health(repository))


@app.command("trade-monitor-status")
def trade_monitor_status(
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
    stale_after_seconds: int = typer.Option(600, help="Heartbeat age threshold for stale status."),
    lock_path: Optional[str] = typer.Option(None, help="Optional monitor lock file path."),
) -> None:
    from tradingagents.trade_lifecycle import TradePlanRepository, monitor_status
    ...
    _ad_print(
        ...
        monitor_status(repository, stale_after_seconds=stale_after_seconds, lock_path=lock_path),
    )


@app.command("trade-monitor-preflight")
def trade_monitor_preflight(
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
    lock_path: Optional[str] = typer.Option(None, help="Optional monitor lock file path."),
) -> None:
    from tradingagents.trade_lifecycle import TradePlanRepository, monitor_preflight

    config = DEFAULT_CONFIG.copy()
    if db_path:
        config["trade_lifecycle_db_path"] = db_path
    repository = TradePlanRepository(config.get("trade_lifecycle_db_path"))
    _ad_print("trade_monitor_preflight", monitor_preflight(repository, config=config, lock_path=lock_path))


@app.command("trade-plan-reconcile")
def trade_plan_reconcile(
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
) -> None:
    from tradingagents.trade_lifecycle import TradePlanRepository, reconcile_plans

    config = DEFAULT_CONFIG.copy()
    if db_path:
        config["trade_lifecycle_db_path"] = db_path
    repository = TradePlanRepository(config.get("trade_lifecycle_db_path"))
    _ad_print("trade_plan_reconcile", reconcile_plans(repository))


@app.command("trade-plan-action")
def trade_plan_action(
    plan_id: str = typer.Option(..., help="Trade plan id."),
    action: str = typer.Option(..., help="Manual action: cancel, reject, mark-executed, mark-no-execution, needs-reconciliation."),
    reason: str = typer.Option("", help="Human-readable reason for the manual action."),
    db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
) -> None:
    from tradingagents.trade_lifecycle import TradePlanRepository, record_manual_action, summarize_plan

    config = DEFAULT_CONFIG.copy()
    if db_path:
        config["trade_lifecycle_db_path"] = db_path
    repository = TradePlanRepository(config.get("trade_lifecycle_db_path"))
    plan = record_manual_action(repository, plan_id=plan_id, action=action, reason=reason)
    if not plan:
        raise typer.BadParameter(f"unknown plan_id: {plan_id}")
    _ad_print("trade_plan_action", summarize_plan(plan, repository))
...
def eval_target_build(
    ...
    trade_db_path: Optional[str] = typer.Option(None, help="Optional trade lifecycle SQLite path."),
    ad_db_path: Optional[str] = typer.Option(None, help="Optional AlphaDiscovery SQLite path."),
) -> None:
    from tradingagents.eval import EpisodeLedger
    from tradingagents.trade_lifecycle import TradePlanRepository
    ...
    trade_repository = (
        TradePlanRepository(trade_db_path or DEFAULT_CONFIG.get("trade_lifecycle_db_path"))
        if include_trade_plans
        else None
    )
    alpha_repository = (
        AlphaDiscoveryRepository(ad_db_path or DEFAULT_CONFIG.get("alpha_discovery_db_path"))
    ...
    _ad_print("eval_target_build", {"created_or_updated": len(targets), "target_ids": [target.target_id for target in targets]})
...
def eval_target_list(
    ...
    pending_only: bool = typer.Option(False, help="Only targets missing current reward version outcome."),
    ...
    _ad_print("eval_target_list", rows)
...
def eval_target_resolve(
    ...
    _ad_print("eval_target_resolve", {"resolved": len(outcomes), "target_ids": [outcome.target_id for outcome in outcomes]})
        ...
        json.dumps(
            {
                ...
                "monitor_windows": [
                    {
                        "time": "market-hours-every-1-5-min",
                        "purpose": "scan active conditional trade plans and run validator/paper execution when triggered",
                    }
                ],
                "plan_health_windows": [
                    {"time": "09:20", "purpose": "pre-open lifecycle health and stale/reconciliation check"},
                    {"time": "16:10", "purpose": "post-close lifecycle summary"},
                ],
                ...
                "commands": {
                    ...
                    "monitor_once": "python -m cli.main trade-monitor --once",
                    "plan_health": "python -m cli.main trade-plan-health",
                    "plan_reconcile": "python -m cli.main trade-plan-reconcile",
                    "plan_list": "python -m cli.main trade-plan-list --status active,triggered,needs_reconciliation",
...
def run_index(
    ...
    action: Optional[str] = typer.Option(None, help="Final action filter, e.g. BUY/HOLD/SELL."),
    signal: Optional[str] = typer.Option(None, help="Final signal filter, e.g. BUY/HOLD/SELL."),
    ...
    limit: Optional[int] = typer.Option(None, help="Maximum indexed runs to show."),
    ...
    filters = {
        ...
        "until": until,
        "horizon": horizon,
        "final_action": action.upper() if action else None,
        "final_signal": signal.upper() if signal else None,
...
@app.command("buy-runs")
def buy_runs(
    symbol: Optional[str] = typer.Option(None, help="Symbol filter."),
    since: Optional[str] = typer.Option(None, help="Only include trade_date >= YYYY-MM-DD."),
    until: Optional[str] = typer.Option(None, help="Only include trade_date <= YYYY-MM-DD."),
    recent: int = typer.Option(20, help="Maximum BUY runs to show."),
    include_high_leakage: bool = typer.Option(True, help="Include high-leakage historical runs."),
    format: str = typer.Option("table", help="Output format: table or json."),
) -> None:
    """Show recent historical runs whose final action is BUY."""
    ledger = _safe_ledger(DEFAULT_CONFIG)
    if ledger is None:
        raise typer.BadParameter("Episode ledger unavailable.")
    ...
    table = Table(title="Recent BUY Runs", box=box.SIMPLE)
    for column in ("run_id", "symbol", "date", "horizon", "signal", "confidence", "quality"):
        table.add_column(column)
    for item in records:
        table.add_row(
            str(item.get("run_id")),
