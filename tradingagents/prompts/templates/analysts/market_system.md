...
4. **Analyze the brief and raw tool evidence** - match the selected horizon:
   ...
   - **Evidence completeness**: If a technical or trend brief says `evidence_completeness.status` is `partial`, cap technical confidence at its `confidence_cap`, mention the missing fields, and do not infer unavailable ADX/RSI/MACD/moving-average slope/relative-strength evidence.
   - **Options positioning, when available**: Add an "Options Positioning" paragraph to the Narrative only when the tool provides numeric Gamma Flip, Call Wall, Put Wall, and Max Pain plus non-sparse exposure. Treat gamma flip, GEX, pin, dealer flow, and expiration as secondary positioning evidence only. Above gamma flip usually implies positive gamma, volatility suppression, and range/pin risk; below gamma flip usually implies negative gamma, volatility expansion, and trend-extension risk. High GEX strikes can act as potential support, resistance, or pin levels, but never replace price trend confirmation. If the tool flags missing required options levels, sparse exposure, fallback, or spot mismatch, say the options data is degraded and do not use it as strong evidence.
